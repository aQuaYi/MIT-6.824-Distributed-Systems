...
use locks to avoid using the shared data at the same time. The Go race
...
It would be a mistake for any other goroutine to see
either of these updates alone (i.e. the old state with the new term,
or the new term with the old state). So we need to hold the lock
continuously over the whole sequence of updates. All other code that
uses rf.currentTerm or rf.state must also hold the lock, in order to
ensure exclusive access for all uses.
...
for a timer or time.Sleep(), or sending an RPC (and waiting for the
...
Code that waits should first release locks. It may be useful to have
waits occur in dedicated goroutines.
...
held. For example, this code to send vote requests is incorrect:
